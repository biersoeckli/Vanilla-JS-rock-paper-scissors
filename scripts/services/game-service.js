    ...
    if (isConnected()) {
        ...
        Object.keys(historyByUser)
            .forEach((playerName) => {
                ...
                const sumOfWins = playerHistory
                    .filter((historyItem) => historyItem.points === 1).length;
                const sumOfLost = playerHistory
                    .filter((historyItem) => historyItem.points === -1).length;
...
export function getRankings(rankingsCallbackHandlerFn) {
    getRankingsFromPlayerStats()
        .then((rankingsArray) => rankingsCallbackHandlerFn(rankingsArray));
}
